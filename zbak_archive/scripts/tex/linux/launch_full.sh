...
kill_listener_on_port() {
  ...
  while IFS= read -r pid; do
    [[ -n "${pid}" ]] || continue
    echo "[launch_full] Stopping stray listener on port ${port} (PID ${pid})..."
...
validate_pid_file_running() {
  ...
  if [[ ! -f "${pid_file}" ]]; then
    echo "[launch_full] ${name} did not create a pid file."
    return 1
  fi
  ...
  if [[ -z "${pid}" ]] || ! kill -0 "${pid}" >/dev/null 2>&1; then
    echo "[launch_full] ${name} failed to stay running."
    if [[ -f "${LOG_DIR}/${name}.log" ]]; then
      echo "[launch_full] Last ${name}.log lines:"
...
if [[ ! -f "${HELIX_KAFKA_UI_JAR}" ]]; then
  echo "[launch_full] Downloading Kafka UI..."
...
echo "[launch_full] Starting RabbitMQ..."
brew services start rabbitmq

echo "[launch_full] Starting Kafka..."
brew services start kafka

if [[ -f "${HELIX_KAFKA_UI_PID_FILE}" ]] && kill -0 "$(cat "${HELIX_KAFKA_UI_PID_FILE}")" >/dev/null 2>&1; then
  echo "[launch_full] Kafka UI already running with PID $(cat "${HELIX_KAFKA_UI_PID_FILE}")"
else
  echo "[launch_full] Starting Kafka UI..."
  env \
...
echo "[launch_full] Waiting for brokers..."
sleep 5

"${SCRIPT_DIR}/../../demo/linux/clean.sh"

echo "[launch_full] Building helix-rest production publish..."
"${SCRIPT_DIR}/rest_build.sh"

echo "[launch_full] Building helix-web production bundle..."
"${SCRIPT_DIR}/web_build.sh"

export ASPNETCORE_ENVIRONMENT=Production

echo "[launch_full] Starting helix-rest..."
...
echo "[launch_full] Starting helix-runtime..."
...
echo "[launch_full] Starting helix-web..."
...
echo "[launch_full] Helix is starting."
echo "[launch_full] REST: ${HELIX_API_URL}"
echo "[launch_full] Web: ${HELIX_WEB_URL}"
echo "[launch_full] RabbitMQ UI: ${HELIX_RABBITMQ_MANAGEMENT_URL}"
echo "[launch_full] Kafka UI: ${HELIX_KAFKA_UI_URL}"
echo "[launch_full] Logs:"
