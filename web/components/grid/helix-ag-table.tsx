...
export function HelixAgTable<T extends Record<string, unknown>>({
  ...
  const isAutoHeight =
    height === "auto" || gridOptions?.domLayout === "autoHeight";
  ...
  function handleCellKeyDown(event: CellKeyDownEvent<T>) {
    onCellKeyDown?.(event);

    const ev = event.event as KeyboardEvent | undefined;
    if (!ev) {
      return;
    }
  ...
  function setEqualsForTextFilter(colId: string, filterText?: string) {
    ...
    const model = (api.getFilterModel() as Record<
      string,
      { filter?: string; type?: string; filterType?: string }
    >) ?? {};
  ...
  function handlePasteCapture(ev: ReactClipboardEvent) {
    const colId = findFloatingFilterColumnId(ev.target);
    if (!colId) {
      return;
    }
